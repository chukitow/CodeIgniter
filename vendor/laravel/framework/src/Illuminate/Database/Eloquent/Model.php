...
abstract class Model implements ArrayAccess, ArrayableInterface, JsonableInterface, JsonSerializable {
	...
	/**
	 * The array that contains the validation rules
	 *
	 *  @var array
	 */
	 ...
	 *  @var array
	 */
	protected $validates_matches_of = array();
	...
	/**
	 * validates the rules of the model
	 *
	 * @return bool
	 */
	public function valid()
	{
		...
		if (is_array($this->validates_presence_of)) {
			foreach ($this->validates_presence_of as $index => $field) {
				if(array_key_exists($field , $this->attributes)){

					if (!$this->required($this->attributes[$field])) {
						$valid = false;
					...
					$valid = false;
					$this->errors[$field][] = "can't be blank";
					if (array_key_exists("message" , $this->validates_presence_of)) {
						$this->errors[$field][] = $validates_presence_of["message"];
					}
				}
			}
		}
		...
		if (is_array($this->validates_uniqueness_of)) {
			foreach ($this->validates_uniqueness_of as $index => $field) {
				if(array_key_exists($field , $this->attributes)){

					$count = static::where(array($field => $this->attributes[$field]))->count();
					if ($count > 0) {
						
						$valid = false;
						$this->errors[$field][] = "should be unique";
						if (array_key_exists("message" , $this->validates_presence_of)) {
							$this->errors[$field][] = $validates_presence_of["message"];
						}
					}
		...
		//validates_matches_of
		if (is_array($this->validates_matches_of)) {
			foreach ($this->validates_matches_of as $index => $field) {
				if(array_key_exists($field , $this->attributes)){
					
					if (!($this->attributes[$field] === $this->attributes["match_".$field])) {
						$valid = false;
						$this->errors[$field][] = "do not matches";
					}
